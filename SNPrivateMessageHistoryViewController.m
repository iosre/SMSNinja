...
- (void)bulkDelete
{
	...
	NSUInteger count = [idArray count];
	NSMutableArray *insertIndexPaths = [NSMutableArray arrayWithCapacity:50];
	for (NSUInteger i = count; i < [idArray count]; i++)
	{
...
- (void)loadDatabaseSegment
{
	...
	if (openResult == SQLITE_OK)
	{
		NSString *sql = [NSString stringWithFormat:@"select name, content, time, number, id, pictures from privatesms order by (cast(id as integer)) desc limit %lu, 50", (unsigned long)[idArray count]];
		...
		sql = @"select count(*) from privatesms";

...
- (void)bulkDelete
{
	...
	if (openResult == SQLITE_OK)
	{
		...
		{
			NSString *sql = [NSString stringWithFormat:@"delete from blockedsms where number = '%@' and name = '%@' and time = '%@' and content = '%@' and read = '%@' and id = '%@' and pictures = '%@'", numberArray[chosenRowIndexPath.row], [nameArray[chosenRowIndexPath.row] stringByReplacingOccurrencesOfString:@"'" withString:@"''"], timeArray[chosenRowIndexPath.row], [contentArray[chosenRowIndexPath.row] stringByReplacingOccurrencesOfString:@"'" withString:@"''"], readArray[chosenRowIndexPath.row], idArray[chosenRowIndexPath.row], picturesArray[chosenRowIndexPath.row]];
			...
			{
				[fileManager removeItemAtPath:[[PICTURES stringByAppendingString:idArray[chosenRowIndexPath.row]] stringByAppendingFormat:@"-%d.png", i] error:&error];
				if (error) NSLog(@"SMSNinja: Failed to delete %@, error %@", [[PICTURES stringByAppendingString:idArray[chosenRowIndexPath.row]] stringByAppendingFormat:@"-%d.png", i], [error localizedDescription]);
			}
		}
...
- (void)bulkUnread
{
	...
	dispatch_sync(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
		...
		if (openResult == SQLITE_OK)
		{
			...
			{
				NSString *sql = [NSString stringWithFormat:@"update blockedsms set read = '0' where id = '%@'", weakSelf->idArray[chosenRowIndexPath.row]];
				int execResult = sqlite3_exec(database, [sql UTF8String], NULL, NULL, NULL);
				if (execResult != SQLITE_OK) NSLog(@"SMSNinja: Failed to exec %@, error %d", sql, execResult);
...
- (void)bulkRead
{
	...
	dispatch_sync(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
		...
		if (openResult == SQLITE_OK)
		{
			for (NSIndexPath *chosenRowIndexPath in weakSelf->bulkSet)
			{
				NSString *sql = [NSString stringWithFormat:@"update blockedsms set read = '1' where id = '%@'", weakSelf->idArray[chosenRowIndexPath.row]];
				int execResult = sqlite3_exec(database, [sql UTF8String], NULL, NULL, NULL);
				if (execResult != SQLITE_OK) NSLog(@"SMSNinja: Failed to exec %@, error %d", sql, execResult);

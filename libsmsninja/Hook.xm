...
- (Class)loadPluginBundle:(NSBundle *)bundle
{
	...
	if ([bundleIdentifier isEqualToString:@"com.apple.mobilephone.incomingcall"]) %init(SNIncomingCallHook);
	return result;
}
%end
...
%ctor
{
	if (![[[NSProcessInfo processInfo] processName] isEqualToString:@"BBM"])
	{
